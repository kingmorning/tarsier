...
public interface RuleMapper {
	...
	@Select("select wanxin, mobile, email from `persons` where wanxin like  concat('%',#{wanxin},'%') and mobile like  concat('%',#{mobile},'%')")
	List<Map<String, String>> persons(@Param("wanxin")String wanxin, @Param("mobile")String mobile);

	@Select("SELECT distinct project_name projectName FROM `rule` WHERE user_name=#{userName}")
	List<String> projects(String userName);

//	@Select("select wanxin from `persons`")
